...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!studentPath.branchHistory) studentPath.branchHistory = [];
    if (!studentPath.skippedNodes) studentPath.skippedNodes = [];
    if (!studentPath.completedNodes) studentPath.completedNodes = [];
    if (!studentPath.suggestedPath) studentPath.suggestedPath = [];
    ...
    switch (action) {
      ...
      case 'skip_module': {
        ...
        if (!currentNodeId) {
          return NextResponse.json<ApiResponse>(
            { success: false, error: 'Current node ID not set' },
            { status: 400 }
          );
        }
